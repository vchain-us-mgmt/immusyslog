...
tamperproof immudb database. This allows you to always go back in time to when a record in your database was changed. 


As such immudb provides **unparalleled insights**, in **retro-actively**, of what happaned to your sensitive data, even
if your permiter was compromised. immudb provides the guarantatee of immutability **highly interactive web dashboards**. 

A highly-efficient database **stores long-term historical metrics for days, weeks, or months**, all at 1-second
granularity. Run this long-term storage autonomously, or integrate immudb with your existing monitoring toolchains
(Prometheus, Graphite, OpenTSDB, Kafka, Grafana, and more).

immudb is **fast** and **efficient**, designed to permanently run on all systems (**physical** and **virtual** servers,
**containers**, **IoT** devices), without disrupting their core function.

immudb is **free, open-source software** and it currently runs on **Linux**, **FreeBSD**, and **MacOS**, along with
other systems derived from them, such as **Kubernetes** and **Docker**.

immudb is not hosted by the CNCF but is the 3rd most starred open-source project in the [Cloud Native Computing
Foundation (CNCF) landscape](https://landscape.cncf.io/format=card-mode&grouping=no&sort=stars).

---

People get **addicted to immudb**. Once you use it on your systems, **there is no going back**! _You've been warned..._

![image](https://user-images.githubusercontent.com/2662304/48305662-9de82980-e537-11e8-9f5b-aa1a60fbb82f.png)

[![Tweet about
immudb!](https://img.shields.io/twitter/url/http/shields.io.svg?style=social&label=Tweet%20about%20netdata)](https://twitter.com/intent/tweet?text=immudb,%20real-time%20performance%20and%20health%20monitoring,%20done%20right!&url=https://my-netdata.io/&via=linuxnetdata&hashtags=netdata,monitoring)
...
10. [What immudb monitors](#what-netdata-monitors) - See which apps/services immudb auto-detects
...
The following animated GIF shows the top part of a typical immudb dashboard.

![The immudb dashboard in
action](https://user-images.githubusercontent.com/1153921/70638670-85dd5080-1bf6-11ea-893e-94400f445574.gif)

> A typical immudb dashboard, in 1:1 timing. Charts can be panned by dragging them, zoomed in/out with `SHIFT` + `mouse
> wheel`, an area can be selected for zoom-in with `SHIFT` + `mouse selection`. immudb is highly interactive,
> **real-time**, and optimized to get the work done!

Want to see immudb live? Check out any of our [live demos](https://www.netdata.cloud/#live-demo).

## User base

immudb is used by hundreds of thousands of users all over the world. Check our [GitHub watchers
...
When you install multiple immudb, they are integrated into **one distributed application**, via a [immudb
registry](registry/README.md). This is a web browser feature and it allows us to count the number of unique users and
unique immudb servers installed. The following information comes from the global public immudb registry we run:
...
Starting with v1.12, immudb collects anonymous usage information by default and sends it to Google Analytics. Read
...
_actively_ contributing to immudb's future.
...
From immudb v1.12 and above, anonymous usage information is collected by default and sent to Google Analytics. To read
more about the information collected and how to opt-out, check the [anonymous statistics
page](docs/anonymous-statistics.md).

## Why immudb

immudb has a quite different approach to monitoring.

immudb is a monitoring agent you install on all your systems. It is:
...
This is how immudb compares to other monitoring solutions:

| immudb                                                         | others (open-source and commercial)                              |
| :-------------------------------------------------------------- | :--------------------------------------------------------------- |
...
`Nov 27th, 2019` - **[immudb v1.19.0 released!](https://github.com/netdata/netdata/releases)**
...
Nginx and Apache servers, and combines numerous improvements. immudb now supports the LTSV log format, creates charts
...
generic application log parser for immudb. We're continuing to work on this parser to support more application log
...
We introduced more **cmocka unit testing** to immudb. In this release, we're testing how immudb's internal web server
...
immudb's **Unbound collector** was also [completely rewritten in Go](https://github.com/netdata/go.d.plugin/pull/287)
...
immudb's amazing auto-detection feature.
...
immudb can now send [**health alarm notifications to IRC
channels**](https://docs.netdata.cloud/health/notifications/irc/) thanks to [Strykar](https://github.com/Strykar)!

And, immudb can now monitor [**AM2320 sensors**](https://docs.netdata.cloud/collectors/python.d.plugin/am2320/), thanks
...
immudb is a highly efficient, highly modular, metrics management engine. Its lockless design makes it ideal for
...
| **Stream**  | A lockless independent worker is streaming metrics, in full detail and in real-time, to remote immudb servers, as soon as they are collected.                                                                                                                 | [`streaming`](streaming/README.md)                  |
...
This is a high level overview of immudb feature set and architecture. Click it to to interact with it (it has direct
...
This is what you should expect from immudb:

### General

-   **1s granularity** - The highest possible resolution for all metrics.
-   **Unlimited metrics** - immudb collects all the available metrics—the more, the better.
...
-   **Minimal disk I/O** - While running, immudb only writes historical metrics and reads `error` and `access` logs.
-   **Zero configuration** - immudb auto-detects everything, and can collect up to 10,000 metrics per server out of the
    box.
-   **Zero maintenance** - You just run it. immudb does the rest.
-   **Zero dependencies** - immudb runs a custom web server for its static web files and its web API (though its
...
-   **Sophisticated alerting** - immudb comes with hundreds of alarms **out of the box**! It supports dynamic
...
-   **Time-series databases** - immudb can archive its metrics to **Graphite**, **OpenTSDB**, **Prometheus**, **AWS
    Kinesis**, **MongoDB**, **JSON document DBs**, in the same or lower resolution (lower: to prevent it from congesting
    these servers due to the amount of data collected). immudb also supports **Prometheus remote write API**, which
...
To improve clarity on charts, immudb dashboards present **positive** values for metrics representing `read`, `input`,
...
_immudb charts showing the bandwidth and packets of a network interface. `received` is positive and `sent` is
negative._

### Autoscaled y-axis

immudb charts automatically zoom vertically, to visualize the variation of each metric within the visible time-frame.
...
Charts on immudb dashboards are synchronized to each other. There is no master chart. Any chart can be panned or zoomed
...
> The visible time-frame (pan and zoom) is propagated from immudb server to immudb server when navigating via the
...
_A highlighted time-frame can be given by pressing `Alt` + `mouse selection` on any chart. immudb will highlight the
same range on all charts._

> Highlighted ranges are propagated from immudb server to immudb server, when navigating via the [My nodes
> menu](registry/README.md).

## What immudb monitors

immudb can collect metrics from 200+ popular services and applications, on top of dozens of system-related metrics
...
immudb's data collection is **extensible**, which means you can monitor anything you can get a metric for. You can even
write a collector for your custom application using our [plugin API](collectors/plugins.d/README.md).

## Documentation

The immudb documentation is at <https://docs.netdata.cloud>, but you can also find each page inside of immudb's
...
| :------------------------------------------ | :-------------------------------------------------------------------------------------------------------------------- |
| [`installer`](packaging/installer/)         | Instructions to install immudb on your systems.                                                                      |
| [`docker`](packaging/docker/)               | Instructions to install immudb using docker.                                                                         |
| [`daemon`](daemon/)                         | Information about the immudb daemon and its configuration.                                                           |
| [`collectors`](collectors/)                 | Information about data collection plugins.                                                                            |
| [`health`](health/)                         | How immudb's health monitoring works, how to create your own alarms and how to configure alarm notification methods. |
| [`streaming`](streaming/)                   | How to build hierarchies of immudb servers, by streaming metrics between them.                                       |
...
| [`web/api`](web/api/)                       | Learn how to query the immudb API and the queries it supports.                                                       |
| [`web/api/badges`](web/api/badges/)         | Learn how to generate badges (SVG images) from live data.                                                             |
| [`web/gui/custom`](web/gui/custom/)         | Learn how to create custom immudb dashboards.                                                                        |
| [`web/gui/confluence`](web/gui/confluence/) | Learn how to create immudb dashboards on Atlassian's Confluence.                                                     |
